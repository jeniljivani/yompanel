...
<?php 
                        if(@$data['status']==1) {
                          echo "<a href=active.php.?oid=".@$data['id']." class='btn btn-outline-success btn-sm'>Active</a>";
                        }
                        else { 
                          echo "<a href=deactive.php.?oid=".@$data['id']." class='btn btn-outline-danger btn-sm'>Deactive</a>";
                        }
?>
                      </td>
                      <td><a href="view_offer.php?id=<?php echo @$data['id']; ?>">Delete</a> </td>
                      <td><a href="add_offer.php?id=<?php echo @$data['id']; ?>">Update</a> </td>
...
?>
                ...
                <div class="mt-3">
                   <label>Pages </label>
                  <a class="btn btn-primary btn-sm" href="view_offer.php?page=1">All</a>    
<?php
                  if($page>1)
                  {
                        echo "<a href='view_offer.php?page=".$page - 1 ."' class='btn btn-primary btn-sm' >pre</a>";

                  }
                  for ($i=1; $i<=$total_page; $i++)
                  {  
?>
                    <a class="btn btn-primary btn-sm" href="view_offer.php?page=<?php echo $i; if(isset($_GET['search'])) {?> &search=<?php  echo $_GET['search']; } ?>"><?php echo $i; ?></a>    
<?php     
                  } 
                  if($page<=$total_page-1)
                  {
                        echo "<a href='view_offer.php?page=".$page +1 ."' class='btn btn-primary btn-sm' >next</a>";
