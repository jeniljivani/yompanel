<?php 
 ...
 $limit = 2;
  ...
  {
    $search = $_GET['search'];
    $sql_page = "select * from `slider` where titel like '%$search%'  limit $start , $limit";
    $total_rec = "select * from `slider` where titel like '%$search%' ";
...
<?php 
                        if(@$data['status']==1) {
                          echo "<a href=active.php.?id=".@$data['id']." class='btn btn-outline-success btn-sm'>Active</a>";
                        }
                        else { 
                          echo "<a href=deactive.php.?id=".@$data['id']." class='btn btn-outline-danger btn-sm'>Deactive</a>";
                        }
?>
                      </td>
...
<?php
                  ...
                  {  
?>
                    <a class="btn btn-primary btn-sm" href="view_slider.php?page=<?php echo $i; if(isset($_GET['search'])) {?> &search=<?php  echo $_GET['search']; } ?>"><?php echo $i; ?></a>    
<?php     
                  } 
