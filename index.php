<?php 
	...
	// offer select ***********
	$offer_select = "select * from `offer` where `status`=1 order by `id` desc limit 3";
	$offer_res = mysqli_query($con, $offer_select);

	// recent photos select ***********
	$recent_select = "select * from `posts` where `status`=1 order by `id` desc";
...
?>
								...
								<img src="admin/image/slider/<?php echo @$slider['image']; ?>" data-fullwidthcentering="on" alt="slide">
...
?>
									<div class="item">
								  	...
								  	<figure>
				        			<img style="height: 250px" alt="portfolio" src="admin/image/latest_posts/<?php echo @$recent['image']; ?>">
				        				<figcaption>
...
?>
						<div class="blog-item">
							<div class="col-md-4">
								<a href="single_blog.php?id=<?php echo @$id;?>"><img style="height: 250px" src="admin/image/latest_posts/<?php echo @$post['image']; ?>" alt=""></a>
								<h3><a href="single_blog.php?id=<?php echo @$id; ?>"><?php echo @$post['titel'] ?></a></h3>
								<span><a href="#"><?php echo @$post['name']; ?></a> / <a href="#"><?php echo@$post['date']; ?></a> / <a href="#"><?php echo @$post['categori']; ?></a></span>
								<p><?php echo @$post['description']; ?></p>
								<div class="read-more">
									<a href="single_blog.php?id=<?php echo @$id ?>">Read more</a>
								</div>
							</div>
						</div>
<?php  
